...
- [ ] Add a `Makefile` or `justfile` with common commands (`make start`, `make status`, `make logs`)
- [ ] Add a simple Python wrapper script (`litellm-local`) for cross-platform compatibility
- [ ] Document how to add custom models without editing `config.yaml` directly
- [ ] Add a VS Code `settings.json` snippet file for easy Copilot Chat setup
